<template>
	<div>
		...
		<el-card>
			...
			<!-- 添加对话框 -->
			<el-dialog title="添加管理员" :visible.sync="addDialogVisible" width="40%" @close="addFormClose">
				<!-- 主体信息 -->
			...
			<!-- 编辑对话框 -->
			<el-dialog title="编辑管理员" :visible.sync="editDialogVisible" width="40%" @close="editFormClose">
				<!-- 主体信息 -->
		...
		statusEdit(row) {
			...
			this.$confirm('是否继续变更状态?', '提示', {
				...
				.then(async () => {
					...
					// 发起请求
					const { data: res } = await this.$http.put('admin/status', { id: row.id, status: status });
					if (res.code !== 200) return this.$message.error(res.msg);
					this.$message.success(res.msg);
					// 刷新列表
					this.adminList();
