...
export default {
	data() {
		return {
			...
			// 第三方登录绑定
			bindLogin: {
				qq: window.serverConfig.BASE_API + 'oauth/login/' + 'qq',
				weixin: window.serverConfig.BASE_API + 'oauth/login/' + 'weixin',
				weibo: window.serverConfig.BASE_API + 'oauth/login/' + 'sina',
				gitee: window.serverConfig.BASE_API + 'oauth/login/' + 'gitee'
			},
			//表单数据验证
			loginFormRules: {
				user: [
	...
	methods: {
		 ...
		 */
		async getCaptcha() {
			const { data: res } = await this.$http.get('login/captcha');
			// 刷新验证码
			this.$refs.code.setAttribute('src', res.data.base64);
			// 保存验证码md5
			window.sessionStorage.setItem('md5', res.data.md5);
