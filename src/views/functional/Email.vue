...
export default {
  ...
  methods: {
     ...
     */
    async email() {
      const { data: res } = await this.$http.get('functional/email');
    ...
    submit() {
      this.$refs.emailFormRef.validate(async valid => {
        if (!valid) return;
        const { data: res } = await this.$http.put('functional/email', this.emailForm);
        if (res.code !== 200) return this.$message.error(res.msg);
     ...
     */
    async testSend() {
      const { data: res } = await this.$http.post('functional/email', this.emailForm);
      if (res.code !== 200) return this.$message.error(res.msg);
      this.$message.success(res.msg);
    }
  }
