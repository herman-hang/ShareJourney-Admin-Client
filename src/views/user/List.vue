	...
	<div>
		...
		<el-card>
			...
			<el-table :data="userListData" border style="width: 100%" @selection-change="handleSelectionChange">
				...
				<el-table-column prop="is_owner" label="是否为车主">
					<template slot-scope="scope">
						<span v-if="scope.row.is_owner === '2'">是</span>
						<span v-else>否</span>
					</template>
				</el-table-column>
			...
			<!-- 添加对话框 -->
			<el-dialog title="添加用户" :visible.sync="addDialogVisible" width="40%" @close="addFormClose">
				<!-- 主体信息 -->
				<el-form :model="addForm" :rules="addFormRules" ref="addFormRef" label-width="100px" class="form-text">
					...
					<el-form-item label="车主" prop="is_owner">
						<el-select v-model="addForm.is_owner" placeholder="请选择是否成为车主">
							<el-option label="用户" value="0"></el-option>
							<el-option label="车主" value="2"></el-option>
						</el-select>
					</el-form-item>
					<div v-show="addForm.is_owner === '2' ? true : false">
						<el-form-item label="平台服务费" prop="service">
							<el-input placeholder="请输入平台服务费" v-model="addForm.owner.service"></el-input>
							<template slot="append">
								%
							</template>
						</el-form-item>
						<el-form-item label="每公里收费" prop="km">
							<el-input placeholder="请输入每公里收费" v-model="addForm.owner.km"></el-input>
							<template slot="append">
								元
							</template>
						</el-form-item>
						<el-form-item label="驾驶证" prop="patente_url">
							...
							<el-upload
								...
								:headers="headers"
								:show-file-list="false"
								:on-success="handleAvatarSuccessAddPatente"
								:before-upload="beforeAvatarUpload"
								name="image"
							>
								<img v-if="addForm.owner.patente_url" :src="addForm.owner.patente_url" class="avatar" />
								<i v-else class="el-icon-plus avatar-uploader-icon"></i>
							</el-upload>
						</el-form-item>
						<el-form-item label="行驶证" prop="registration_url">
							...
							<el-upload
								...
								:headers="headers"
								:show-file-list="false"
								:on-success="handleAvatarSuccessAddRegistration"
								:before-upload="beforeAvatarUpload"
								name="image"
							>
								<img v-if="addForm.owner.registration_url" :src="addForm.owner.registration_url" class="avatar" />
								<i v-else class="el-icon-plus avatar-uploader-icon"></i>
							</el-upload>
						</el-form-item>
						<el-form-item label="车辆图片" prop="car_url">
							...
							<el-upload
								...
								:headers="headers"
								:show-file-list="false"
								:on-success="handleAvatarSuccessAddCar"
								:before-upload="beforeAvatarUpload"
								name="image"
							>
								<img v-if="addForm.owner.car_url" :src="addForm.owner.car_url" class="avatar" />
								<i v-else class="el-icon-plus avatar-uploader-icon"></i>
							</el-upload>
						</el-form-item>
						<el-form-item label="车牌号" prop="plate_number"><el-input placeholder="请输入车牌号" v-model="addForm.owner.plate_number"></el-input></el-form-item>
						<el-form-item label="可载人数" prop="capacity"><el-input placeholder="请输入可载人数" v-model="addForm.owner.capacity"></el-input></el-form-item>
						<el-form-item label="车辆颜色" prop="color"><el-input placeholder="请输入车辆颜色" v-model="addForm.owner.color"></el-input></el-form-item>
					</div>
				</el-form>
			...
			<!-- 编辑对话框 -->
			<el-dialog title="编辑用户" :visible.sync="editDialogVisible" width="40%" @close="editFormClose">
				<!-- 主体信息 -->
				<el-form :model="editForm" :rules="editFormRules" ref="editFormRef" label-width="100px" class="form-text">
					...
					<el-form-item label="车主" prop="is_owner">
						<el-select v-model="editForm.is_owner" placeholder="请选择是否成为车主">
							<el-option label="用户" value="0"></el-option>
					...
					<div v-show="editForm.is_owner === '0' ? false : true">
						<el-form-item label="平台服务费" prop="service">
							<el-input placeholder="请输入平台服务费" v-model="editForm.owner.service"></el-input>
							<template slot="append">
								%
							</template>
						</el-form-item>
						<el-form-item label="每公里收费" prop="km">
							<el-input placeholder="请输入每公里收费" v-model="editForm.owner.km"></el-input>
							<template slot="append">
								元
							</template>
						</el-form-item>
						<el-form-item label="驾驶证" prop="patente_url">
							...
							<el-upload
								...
								name="image"
							>
								<img v-if="editForm.owner.patente_url" :src="editForm.owner.patente_url" class="avatar" />
								<i v-else class="el-icon-plus avatar-uploader-icon"></i>
							</el-upload>
						</el-form-item>
						<el-form-item label="行驶证" prop="registration_url">
							...
							<el-upload
								...
								name="image"
							>
								<img v-if="editForm.owner.registration_url" :src="editForm.owner.registration_url" class="avatar" />
								<i v-else class="el-icon-plus avatar-uploader-icon"></i>
							</el-upload>
						</el-form-item>
						<el-form-item label="车辆图片" prop="car_url">
							...
							<el-upload
								...
								name="image"
							>
								<img v-if="editForm.owner.car_url" :src="editForm.owner.car_url" class="avatar" />
								<i v-else class="el-icon-plus avatar-uploader-icon"></i>
							</el-upload>
						</el-form-item>
						<el-form-item label="车牌号" prop="plate_number"><el-input placeholder="请输入车牌号" v-model="editForm.owner.plate_number"></el-input></el-form-item>
						<el-form-item label="可载人数" prop="capacity"><el-input placeholder="请输入可载人数" v-model="editForm.owner.capacity"></el-input></el-form-item>
						<el-form-item label="车辆颜色" prop="color"><el-input placeholder="请输入车辆颜色" v-model="editForm.owner.color"></el-input></el-form-item>
					</div>
				</el-form>
				<span slot="footer" class="dialog-footer">
...
export default {
	data() {
		...
		return {
			...
			addFormRules: {
				...
				age: [{ required: true, type: 'number', min: 0, max: 120, message: '年龄只能是0到120岁之间', trigger: 'blur' }],
				mobile: [{ validator: validatePhone, trigger: 'blur' }],
				email: [{ validator: validEmail, trigger: 'blur' }],
				qq: [{ min: 5, max: 11, message: 'QQ号只能是5到11位之间', trigger: 'blur' }],
				status: [{ required: true, message: '请选择状态', trigger: 'change' }],
				is_owner: [{ required: true, message: '请选择是否成为车主', trigger: 'change' }]
			},
			// 编辑表单验证规则
			editFormRules: {
				user: [{ required: true, message: '请输入账号', trigger: 'blur' }, { min: 5, max: 15, message: '账号只能是5到15位之间', trigger: 'blur' }],
	...
	methods: {
		...
		async editDialog(id) {
			...
			if (res.code !== 200) return this.$message.error(res.msg);
			if (res.data.owner === null || res.data.owner === '' || res.data.owner === undefined) {
				res.data.owner = {
					patente_url: '',
					registration_url: '',
					car_url: ''
		 ...
		 * @param {Object} file 文件对象
		 */
		handleAvatarSuccessAddPatente(res, file) {
			if (res.code !== 200) return this.$message.error(res.msg);
			this.addForm.owner.patente_url = res.data[0];
			this.$message.success(res.msg);
		},
		 ...
		 * @param {Object} file 文件对象
		 */
		handleAvatarSuccessAddRegistration(res, file) {
			if (res.code !== 200) return this.$message.error(res.msg);
			this.addForm.owner.registration_url = res.data[0];
			this.$message.success(res.msg);
		},
		 ...
		 * @param {Object} file 文件对象
		 */
		handleAvatarSuccessAddCar(res, file) {
			if (res.code !== 200) return this.$message.error(res.msg);
			this.addForm.owner.car_url = res.data[0];
			this.$message.success(res.msg);
		},
		...
		handleAvatarSuccessEditPatente(res, file) {
			if (res.code !== 200) return this.$message.error(res.msg);
			this.editForm.owner.patente_url = res.data[0];
			this.$message.success(res.msg);
		},
		...
		handleAvatarSuccessEditRegistration(res, file) {
			if (res.code !== 200) return this.$message.error(res.msg);
			this.editForm.owner.registration_url = res.data[0];
			this.$message.success(res.msg);
		},
		...
		handleAvatarSuccessEditCar(res, file) {
			if (res.code !== 200) return this.$message.error(res.msg);
			this.editForm.owner.car_url = res.data[0];
			this.$message.success(res.msg);
		},
		...
		addFormClose() {
			...
			// 清空车主信息对象
			const obj = this.addForm.owner;
			for (const key in obj) {
				this.addForm.owner[key] = '';
			}
		},
		...
		statusEdit(row) {
			...
			this.$confirm('是否继续变更状态?', '提示', {
				...
				.then(async () => {
					...
					// 发起请求
					const { data: res } = await this.$http.put('user/status', { id: row.id, status: status });
					if (res.code !== 200) return this.$message.error(res.msg);
					this.$message.success(res.msg);
					// 刷新列表
					this.userList();
