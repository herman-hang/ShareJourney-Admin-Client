<template>
	<div>
		...
		<el-breadcrumb separator-class="el-icon-arrow-right">
			...
			<el-breadcrumb-item>提现审核列表</el-breadcrumb-item>
		</el-breadcrumb>
			...
			<el-tabs v-model="activeName" @tab-click="handleClick">
				...
				<el-tab-pane label="待审核" name="first">
					<el-table :data="unauditedData" border style="width: 100%">
						...
						<el-table-column prop="owner_id" label="车主ID"></el-table-column>
						...
						<el-table-column prop="withdraw_account" label="审核到账">
							<template slot-scope="scope">
								...
								<el-tag type="danger" v-else>银行卡</el-tag>
							</template>
						</el-table-column>
					...
					<!-- 审核对话框 -->
					<el-dialog title="审核订单" :visible.sync="auditDialogVisible" width="40%" @close="auditFormClose">
						<el-form :model="auditForm" ref="auditFormRef" label-width="120px" class="form-text">
							...
							<el-form-item label="驳回原因:">
								<span v-if="auditForm.status === '2'">{{ auditForm.cause }}</span>
								<span v-else>无</span>
							</el-form-item>
							...
							<el-form-item label="银行卡号:">
								<span>{{ auditForm.bank_card }}</span>
							</el-form-item>
							<el-form-item label="银行卡姓名:">
								<span>{{ auditForm.bank_card_name }}</span>
							</el-form-item>
							<el-form-item label="银行卡类型:">
								<span>{{ auditForm.bank_card_type }}</span>
							</el-form-item>
						</el-form>
				...
				<el-tab-pane label="已审核" name="second">
					...
					<!-- 查看订单对话框 -->
					<el-dialog title="查看订单" :visible.sync="checkInfoDialogVisible" width="40%" @close="checkInfoFormClose">
						<el-form :model="checkInfoForm" ref="checkInfoFormRef" label-width="120px" class="form-text">
							<el-form-item label="订单ID:">
								<span>{{ checkInfoForm.id }}</span>
							</el-form-item>
							<el-form-item label="车主ID:">
								<span>{{ checkInfoForm.onwer_id }}</span>
							</el-form-item>
							<el-form-item label="用户ID:">
								<span>{{ checkInfoForm.user_id }}</span>
							</el-form-item>
							<el-form-item label="用户名:">
							...
							<el-form-item label="驳回原因:">
								<span v-if="checkInfoForm.status === '2'">{{ checkInfoForm.cause }}</span>
								<span v-else>无</span>
							</el-form-item>
							<el-form-item label="钱包余额:">
							...
							<el-form-item label="平台服务费(%):">
								<span>{{ checkInfoForm.service }}</span>
							</el-form-item>
							<el-form-item label="每公里收费(元):">
								<span>{{ checkInfoForm.km }}</span>
							</el-form-item>
							<el-form-item label="手机号码:">
							...
							<el-form-item label="银行卡号:">
								<span>{{ checkInfoForm.bank_card }}</span>
							</el-form-item>
							<el-form-item label="银行卡姓名:">
								<span>{{ checkInfoForm.bank_card_name }}</span>
							</el-form-item>
							<el-form-item label="银行卡类型:">
								<span>{{ checkInfoForm.bank_card_type }}</span>
							</el-form-item>
						</el-form>
					</el-dialog>
				</el-tab-pane>
...
export default {
	...
	methods: {
		...
		async audit(type) {
			if (type === '1') {
				// 待审核
				const { data: res } = await this.$http.get('owner/withdraw/list', { params: this.unauditedQueryInfo });
				if (res.code !== 200) return this.$message.error(res.msg);
				this.unauditedQueryInfo.total = res.data.total;
				this.unauditedData = res.data.data;
			} else {
				// 已审核
				const { data: res } = await this.$http.get('owner/withdraw/list', { params: this.auditQueryInfo });
				if (res.code !== 200) return this.$message.error(res.msg);
		 ...
		 */
		async checkInfo(id) {
			const { data: res } = await this.$http.get(`owner/withdraw/query/${id}`);
			if (res.code !== 200) return this.$message.error(res.msg);
		...
		async pass() {
			...
			const { data: res } = await this.$http.put('owner/withdraw/pass', { id: this.auditForm.id });
			// 关闭加载图标
		...
		reject(id) {
			this.$prompt('请输入驳回原因', '提示', {
				...
				.then(async ({ value }) => {
					...
					this.fullscreenLoading = true;
					// 发起请求
					const { data: res } = await this.$http.put('owner/withdraw/reject', { id: id, cause: value });
					if (res.code !== 200) return this.$message.error(res.msg);
					this.$message.success(res.msg);
					// 显示加载图标
					this.fullscreenLoading = false;
