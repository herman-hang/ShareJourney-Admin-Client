<template>
  <div>
    ...
    <el-card>
      ...
      <!-- 添加对话框 -->
      <el-dialog title="添加权限组" :visible.sync="addDialogVisible" @close="addFormClose" width="40%">
        <!-- 主体信息 -->
      ...
      <!-- 编辑对话框 -->
      <el-dialog title="编辑权限组" :visible.sync="editDialogVisible" @close="editFormClose" width="40%">
        <!-- 主体信息 -->
...
export default {
  ...
  methods: {
    ...
    statusEdit(row) {
      ...
      this.$confirm('是否继续变更状态?', '提示', {
        ...
        .then(async () => {
          ...
          // 发起请求
          const { data: res } = await this.$http.put('group/status', { id: row.id, status: status });
          if (res.code !== 200) return this.$message.error(res.msg);
          this.$message.success(res.msg);
          // 刷新列表
          this.groupList();
    ...
    async editDialog(id) {
      // 调用根据权限组ID查询数据方法
      const { data: res } = await this.$http.get(`group/query/${id}`);
      if (res.code !== 200) return this.$message.error(res.msg);
